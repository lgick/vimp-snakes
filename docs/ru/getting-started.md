...
`manifest.requires` (`scripts/build-game-manifest.js`) — эта игра называет
там `accolades`, ради корон лидеров, — и отказать в пакете может только
движок, слишком старый, чтобы это имя предоставить.
