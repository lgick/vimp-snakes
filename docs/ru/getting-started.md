...
Все пять обязаны проходить с `--determinism`. Два инварианта пропускаются по
замыслу: `roundLifecycle` (у игры нет конца раунда) и в трёх из пяти сценариев
`predictionDrift` (крушение и респавн — законные разовые всплески;
